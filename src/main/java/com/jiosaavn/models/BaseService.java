...
public class BaseService {
  ...
  private Response addHeaders(Interceptor.Chain chain) throws IOException {
    String randomUA =
        UserAgents.USER_AGENTS.get(new Random().nextInt(UserAgents.USER_AGENTS.size()));
  ...
  protected <T> T fetchFromApi(
      String endpoint, Map<String, String> queryParams, Class<T> responseType, ApiContext context)
      throws IOException {
  ...
  protected Artist.ArtistModel artistPayloadTransformation(Artist.ArtistAPIResponseModel artist)
    ...
    model.url =
        artist.permaUrl != null
            ? artist.permaUrl
            : artist.urls != null ? artist.urls.overview : null;
    model.type = artist.type;
    try {
      model.followerCount =
          artist.follower_count != null ? Integer.parseInt(artist.follower_count) : 0;
    ...
    if (artist.similarArtists != null) {
      for (Artist.ArtistAPIResponseModel.SimilarArtist sa : artist.similarArtists) {
        ...
        similar.aka = sa.aka;
        similar.bio =
            sa.bio != null ? JsonUtils.parse(sa.bio, new TypeReference<String>() {}) : null;
  ...
  protected ArtistMap.ArtistMapModel artistMapPayloadTransformation(
      ArtistMap.ArtistMapAPIResponseModel artist) {
  ...
  protected Playlist.PlaylistModel playlistPayLoadTransformation(
      Playlist.PlaylistAPIResponseModel playlist) {
  ...
  protected Search.SearchModel searchPayloadTransformation(Search.SearchAPIResponseModel search) {
    ...
    if (search.topQuery != null) {
      Search.SearchResponseModel<Search.SearchModel.TopQueryItem> topQuerySection =
          new Search.SearchResponseModel<>();
    ...
    if (search.songs != null) {
      Search.SearchResponseModel<Search.SearchModel.Song> songsSection =
          new Search.SearchResponseModel<>();
    ...
    if (search.albums != null) {
      Search.SearchResponseModel<Search.SearchModel.Album> albumsSection =
          new Search.SearchResponseModel<>();
    ...
    if (search.artists != null) {
      Search.SearchResponseModel<Search.SearchModel.Artist> artistsSection =
          new Search.SearchResponseModel<>();
    ...
    if (search.playlists != null) {
      Search.SearchResponseModel<Search.SearchModel.Playlist> playlistsSection =
          new Search.SearchResponseModel<>();
  ...
  private List<ArtistMap.ArtistMapModel> transformArtists(
      List<ArtistMap.ArtistMapAPIResponseModel> sourceList) {
  ...
  protected Album.AlbumModel albumPayloadTransformation(Album.AlbumAPIResponseModel album)
    ...
    if (album.list != null && album.list.isArray()) {
      for (JsonNode songNode : album.list) {
        Song.SongAPIResponseModel song =
            objectMapper.treeToValue(songNode, Song.SongAPIResponseModel.class);
        songs.add(songPayloadTransformation(song));
      }
      model.songs = songs;
    } else {
